...
const sourcePlan=analyzeSvg(readFileSync(new URL('../../public/plans/changdong-parking-annotated.svg',import.meta.url),'utf8'));
it('routes the annotated actual parking plan to all three core approaches without inventing evacuation exits',()=>{expect(sourcePlan.layoutType).toBe('source-traced');for(const id of ['A','B','C']){const path=route(sourcePlan,'entrance',id);expect(path.distance).toBeGreaterThan(0);expect(path.points[0].x).toBeCloseTo(13.23);expect(path.points[0].z).toBeCloseTo(-8.73);expect(path.points.every(p=>canMove(sourcePlan,p))).toBe(true);}expect(evacuation(sourcePlan,'entrance')).toBeNull();expect(sourcePlan.scaleStatus).toBe('estimated');});
describe('parking and emergency navigation',()=>{
 ...
 it('calculates a known shortest path and excludes pedestrian-only exits for cars',()=>{
  expect(route(plan,'entrance','C').distance).toBe(41.5);expect(route(plan,'entrance','exit-west')).toBeNull();expect(route(plan,'missing','A')).toBeNull();
 });
 ...
 it('samples motion by distance without overshooting endpoints',()=>{
  const r=route(plan,'entrance','A');expect(sampleRoute(r,0)).toMatchObject({x:0,z:27,arrived:false});expect(sampleRoute(r,3).z).toBe(24);expect(sampleRoute(r,999)).toMatchObject({x:-30,z:-20.5,arrived:true});expect(sampleRoute(null,0)).toBeNull();expect(sampleRoute(route(plan,'A','A'),0).arrived).toBe(true);
 });
