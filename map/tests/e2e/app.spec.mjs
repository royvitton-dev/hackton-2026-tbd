...
const lab=async page=>{await page.evaluate(()=>__atlas.loadLab('lab'));await expect(page.locator('#route-panel')).toBeVisible();await page.waitForFunction(()=>__atlas.state.screen==='lab'&&__atlas.state.route);};
...
test('EV capacity, heatmap layers, survey import and unavailable public cell data remain explicit',async({page})=>{
 await page.evaluate(async()=>{await __atlas.loadLab('lab');await __atlas.showCharging(__atlas.state.plan);});await expect(page.locator('.ev-candidate')).toHaveCount(8);expect(await page.evaluate(()=>__atlas.scene.car.userData.detailed)).toBe(true);
...
test('golden charging map and mobile controls',async({page})=>{
 await page.evaluate(async()=>{await __atlas.loadLab('lab');await __atlas.showCharging(__atlas.state.plan);});await expect(page.locator('.ev-candidate')).toHaveCount(8);await page.waitForTimeout(600);await expect(page).toHaveScreenshot('charging-overview.png');
...
test('floor collection isolates B1–B4 originals, EV inputs, and apartment data; golden basement',async({page})=>{
 ...
 expect(new Set(images).size).toBe(4);await expect(page).toHaveScreenshot('basement-b4.png');await page.locator('#plan-lab').click();await expect(page.locator('#start')).toBeDisabled();await expect(page.locator('.source-evidence')).toContainText('B4');expect(await page.evaluate(()=>__atlas.state.plan.id)).toBe('multilevel-dogok:3');await page.keyboard.press('w');await page.locator('[data-nav="charging"]').click();await expect(page.locator('.ev-empty')).toBeVisible();expect(await page.evaluate(()=>__atlas.state.charging.survey.planId)).toBe('multilevel-dogok:3');
...
test('primary parking and charging use the same actual B2 source and no demo infrastructure; golden actual',async({page})=>{
 test.setTimeout(150000);await page.locator('[data-nav="lab"]').click();await page.waitForFunction(()=>__atlas.state.plan?.id==='changdong-b2'&&__atlas.state.route);await expect(page.locator('.route-origin small')).toHaveText('B2');await expect(page.locator('.source-evidence img')).toHaveAttribute('src','/plans/changdong-parking-b2.png');await expect(page.locator('#destination option')).toHaveCount(3);
 await page.waitForFunction(()=>__atlas.scene.world.getObjectByName('source-blueprint')?.userData.ready===true);await expect(page).toHaveScreenshot('actual-parking.png');const walls=await page.evaluate(()=>JSON.stringify(__atlas.state.plan.walls));
 await page.locator('[data-source-preview]').click();await expect(page.locator('.source-dialog-image')).toBeVisible();await page.locator('#dialog-close').click();await page.locator('[data-source-overlay]').uncheck();expect(await page.evaluate(()=>__atlas.scene.world.getObjectByName('source-blueprint').visible)).toBe(false);
 await page.locator('[data-nav="charging"]').click();await expect(page.locator('.ev-empty')).toBeVisible();await expect(page.locator('#ev-demo')).toBeHidden();expect(await page.evaluate(()=>__atlas.state.charging.survey.planId)).toBe('changdong-b2');expect(await page.evaluate(()=>JSON.stringify(__atlas.state.plan.walls))).toBe(walls);expect(await page.evaluate(()=>__atlas.state.charging.result.selected)).toEqual([]);await page.waitForFunction(()=>__atlas.scene.world.getObjectByName('source-blueprint')?.userData.ready===true);await expect(page).toHaveScreenshot('actual-charging.png');
 await page.setViewportSize({width:390,height:844});expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBe(390);await expect(page.locator('.ev-sidebar .source-evidence')).toContainText('B2');
});
