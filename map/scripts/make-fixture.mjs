...
for(const [id,x,y,label,role] of [['entrance',42,55,'차량 입구','entrance'],['A',12,3,'101동','building'],['B',72,3,'102동','building'],['C',12,53,'103동','building'],['D',72,53,'104동','building'],['exit-west',2,28,'서측 외부 집결지','exit'],['exit-east',82,28,'동측 외부 집결지','exit']])nodes.push({id,x,y,label,role});
...
for(const [x,y,label] of [[7,2,'101'],[67,2,'102'],[7,49,'103'],[67,49,'104']])lines.push(`<rect id="core-${label}" data-kind="space" data-role="core" data-label="${label}동" x="${x}" y="${y}" width="10" height="5" fill="#709b87"/>`);
