...
export async function createGoogleMap(element,sites,onSelect,{apiKey,mapId='DEMO_MAP_ID'}={}){
 if(!apiKey){
  const iframe=document.createElement('iframe');iframe.title='Google 지도 — 서울';iframe.src='https://maps.google.com/maps?q=Seoul%20South%20Korea&t=m&z=11&output=embed&hl=ko';iframe.referrerPolicy='strict-origin-when-cross-origin';iframe.allowFullscreen=true;element.replaceChildren(iframe);
  return {mode:'embed',focus:site=>iframe.src=`https://maps.google.com/maps?q=${encodeURIComponent(site.address)}&z=17&output=embed&hl=ko`,dispose:()=>iframe.remove()};
 }
 ...
 const models=sites.map(site=>{const root=new THREE.Group();root.matrixAutoUpdate=false;for(const x of [-16,16]){const m=new THREE.Mesh(new THREE.BoxGeometry(24,24,54),new THREE.MeshStandardMaterial({color:0x7e9e88,roughness:.6}));m.position.set(x,0,27);root.add(m);}scene.add(root);return {site,root};});
