...
export function validatePlan(plan) {
  ...
  for (const edge of plan.edges || []) if (!ids.has(edge.from) || !ids.has(edge.to) || !finite(edge.width) || edge.width <= 0) throw new Error('동선 연결이 올바르지 않습니다.');
  return plan;
}
...
export function analyzeSvg(text) {
  ...
  for (const match of text.matchAll(/<(line|rect|circle)\b[^>]*>/g)) {
    ...
    if (a['data-kind']==='node') plan.nodes.push({id:a.id,x:x(a.cx),z:z(a.cy),kind:a['data-role']||'junction',label:a['data-label']||a.id, safe:a['data-safe']==='true'});
