...
for(const id of ['10000901','20000555','10002143','parking-159344','multilevel-dogok']){
  const site=catalog.find(x=>x.id===id);
  for(const [i,a] of site.assets.entries()){
    ...
    output.push({id:`${id}-${i}`,siteId:id,name:site.name+(a.floor?` · ${a.floor}`:''),address:site.address,acquiredAt:site.acquiredAt,buildingType:id==='10000901'?'apartment':id==='20000555'?'house':id==='parking-159344'?'building':'public',source:site.source,publisher:site.publisher,sourceAsset:{...a,file,bytes:bytes.length},location:locations[id]||null,parkingEvidence:site.parkingEvidence||null,scaleStatus:'estimated',metersAcross:id==='multilevel-dogok'?90:40});
  }
}
