...
export class TwinScene {
  constructor(canvas,onFrame){
    ...
    this.world=new THREE.Group();this.routeGroup=new THREE.Group();this.hazardGroup=new THREE.Group();this.chargingGroup=new THREE.Group();this.textGroup=new THREE.Group();this.scene.add(this.world,this.routeGroup,this.hazardGroup,this.chargingGroup,this.textGroup);this.finishes={floor:'concrete',wall:'paint',column:'concrete',stairs:'concrete'};this.showCars=true;
  ...
  clear(){dispose(this.world);dispose(this.routeGroup);dispose(this.hazardGroup);dispose(this.chargingGroup);dispose(this.textGroup);this.actor.visible=false;this.view='orbit';this.controls.enabled=true;this.canvas.dataset.ready='false';}
  ...
  showPlan(plan,model,sourceUrl){
    this.clear();this.parkingTarget=null;this.materials=new Map();this.plan=plan;this.world.userData.mode='plan';this.world.userData.meshCount=model.meshes.length;
    ...
    for(const data of model.meshes){const geometry=new THREE.BufferGeometry();geometry.setAttribute('position',new THREE.Float32BufferAttribute(data.positions,3));geometry.setIndex(data.indices);geometry.computeVertexNormals();const mesh=new THREE.Mesh(worldUV(geometry),surfaceMaterial(this.finishes.wall,this.materials));mesh.userData.surface='wall';mesh.castShadow=true;mesh.receiveShadow=true;this.world.add(mesh);}
  ...
  dispose(){this.renderer.setAnimationLoop(null);this.observer.disconnect();this.controls.dispose();for(const g of [this.world,this.routeGroup,this.hazardGroup,this.actor,this.chargingGroup,this.textGroup])dispose(g);this.renderer.dispose();}
}
