...
export function parkedCar(space,index=0){
 ...
 const horizontal=space.width>space.depth;g.rotation.y=horizontal?Math.PI/2:0;const scale=Math.min(1,(Math.max(space.width,space.depth)-.5)/4.3,(Math.min(space.width,space.depth)-.35)/1.8);g.scale.setScalar(Math.max(.25,scale));g.position.set(space.x,(space.y||0)+.06,space.z);g.userData.kind='parked-car';g.userData.spaceId=space.id;g.userData.illustrative=true;return g;
}
export function accessibleMark(space){
 ...
 const texture=new THREE.CanvasTexture(canvas);texture.colorSpace=THREE.SRGBColorSpace;const mesh=new THREE.Mesh(new THREE.PlaneGeometry(space.width,space.depth),new THREE.MeshBasicMaterial({map:texture,side:THREE.DoubleSide}));mesh.rotation.x=-Math.PI/2;mesh.position.set(space.x,(space.y||0)+.11,space.z);mesh.userData.kind='accessible-mark';return mesh;
}
export function semanticObject(data,{column='concrete',stairs='concrete'}={},materialCache){
 ...
 const accent=new THREE.MeshStandardMaterial({color:STRUCTURE_COLORS[data.kind]||'#777f7c',roughness:.55});
 ...
 if(data.kind==='stairs'){
  ...
  for(let i=0;i<n;i++)for(const side of [-1,1]){const upper=side>0,h=rise*(upper?n+i+1:i+1),z=upper?run/2-(i+.5)*tread:-run/2+(i+.5)*tread,step=box(flight,h,tread,surface('stairs'),side*data.width*.25,h/2,z);step.userData.surface='stairs';g.add(step);g.add(box(flight,.025,Math.min(.09,tread*.35),accent,side*data.width*.25,h+.014,z+(upper?1:-1)*(tread/2-.045)));}
 ...
 if(data.kind==='lift'){g.add(box(data.width,data.height,data.depth,new THREE.MeshStandardMaterial({color:'#afbab5',roughness:.7})));g.add(box(data.width+.08,.16,data.depth+.08,accent,0,data.height+.08));for(const x of [-.22,.22])g.add(box(data.width*.43,data.height*.8,.045,steel,x*data.width,data.height*.43,data.depth/2+.03));}
 if(data.kind==='door'){
  g.rotation.y=data.angle||0;
  for(const x of [-1,1])g.add(box(.1,data.height+.12,.18,accent,x*(data.width/2+.05),(data.height+.12)/2));
  g.add(box(data.width+.2,.12,.18,accent,0,data.height+.06));
  const hinge=new THREE.Group();hinge.position.x=-data.width/2;hinge.rotation.y=data.openAngle||0;
  const glass=new THREE.MeshStandardMaterial({color:STRUCTURE_COLORS.door,transparent:true,opacity:.45,roughness:.2});
  hinge.add(box(data.width,data.height,.06,glass,data.width/2,data.height/2));
  for(const x of [0,data.width])hinge.add(box(.045,data.height,.08,accent,x));
  for(const y of [.04,data.height-.04])hinge.add(box(data.width,.08,.08,accent,data.width/2,y));
  hinge.add(box(.14,.05,.13,steel,data.width*.85,1,.07));g.add(hinge);
  const arc=new THREE.BufferGeometry().setFromPoints(Array.from({length:25},(_,i)=>{const a=(data.openAngle||Math.PI/2)*i/24;return new THREE.Vector3(-data.width/2+Math.cos(a)*data.width,.08,-Math.sin(a)*data.width);}));
  const swing=new THREE.Line(arc,new THREE.LineDashedMaterial({color:STRUCTURE_COLORS.door,dashSize:.12,gapSize:.08}));swing.computeLineDistances();g.add(swing);
 }
 ...
 if(data.kind==='ramp'){
  const curve=new THREE.CurvePath();for(let i=1;i<data.path.length;i++){const a=data.path[i-1],b=data.path[i];curve.add(new THREE.LineCurve3(new THREE.Vector3(a.x-data.x,a.y-(data.y||0),a.z-data.z),new THREE.Vector3(b.x-data.x,b.y-(data.y||0),b.z-data.z)));}const positions=[],indices=[],left=[],right=[];
  ...
  const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geo.setIndex(indices);geo.computeVertexNormals();const mat=surface('stairs').clone();mat.side=THREE.DoubleSide;const ramp=new THREE.Mesh(worldUV(geo),mat);ramp.userData.surface='stairs';ramp.userData.doubleSide=true;ramp.receiveShadow=true;g.add(ramp);for(const points of [left,right])g.add(new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points),32,.06,6,false),accent));
 }
 return g;
}
