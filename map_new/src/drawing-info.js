...
export function drawingInfo(plan,site,radio){
 ...
 <details><summary>주차면 진입 연결 · ${(plan.parkingAccess||[]).length}개</summary><p>원본에서 확인한 인접 차로와 주차 구획의 연결입니다. 차폭·차길이·회전반경으로 전진 경로를 계산하고 벽·기둥·보호 구역을 확인합니다. 실측 경로나 후진 주차 안내는 아닙니다.</p><ul>${(plan.parkingAccess||[]).map(a=>`<li>${esc(a.nodeId)} → ${esc(a.spaceId)}</li>`).join('')||'<li>원본에서 확인된 주차면 진입 연결이 없습니다.</li>'}</ul></details>
