...
**현재 범위:** 실제 장소 45곳·9개 유형·공개 도면 63장 + 별도의 합성 통합 시나리오, 원본 SVG/PNG/JPG/PDF와 해시, 3D 벽체 메시, 의미가 명시된 SVG의 Node/Graph 변환. 보관 도면은 구조선 후보와 한국어·영어 OCR 1,274개를 제공합니다. 원본을 검토한 4장에는 주차 구획 188면, 5장에는 기둥·계단·승강기·문·실·램프 134개를 추가했습니다. 문·안전 출구가 확인되지 않은 도면은 이동 경로를 생성하지 않습니다. 내 도면 가져오기는 SVG/분석 JSON/이미지/PDF 첫 페이지를 처리합니다.
...
동북권 B2는 원본 표기 121면 중 확인한 105면과 공간 오브젝트 87개를 모델링했습니다. 작은 표기·미해석 부분을 완성된 구획으로 간주하지 않습니다. 상세 정보에서 모든 주차면·오브젝트의 평면 치수, 가정 높이, 좌표, OCR 원문과 출처를 확인하고 모델 데이터로 내보낼 수 있습니다.
