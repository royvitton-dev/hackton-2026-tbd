...
**현재 범위:** 실제 장소 45곳·9개 유형·공개 도면 63장 + 별도의 합성 통합 시나리오, 원본 SVG/PNG/JPG/PDF와 해시, 3D 벽체 메시, 의미가 명시된 SVG의 Node/Graph 변환. 보관 도면은 구조선 후보와 한국어·영어 OCR 1,506개를 제공합니다. 원본을 검토한 4장에는 주차 구획 187면, 6장에는 기둥·계단·승강기·문·실·램프 183개를 추가했습니다. 문·안전 출구가 확인되지 않은 도면은 이동 경로를 생성하지 않습니다. 내 도면 가져오기는 SVG/분석 JSON/이미지/PDF 첫 페이지를 처리합니다.
...
너나들이 원본의 주차면 3곳에는 확인한 인접 차로 연결을 저장했습니다. 목적지의 `구획 안까지`를 선택하면 차폭·차길이·회전반경에 따른 전진 경로를 계산합니다. 작은 차량의 예시 제원(1.7 × 4.1m, 반경 4.2m)에서는 P2·P3의 구획 내부까지 도달합니다. 기본 제원으로 가능한 전진 경로가 없으면 안내를 중단하며, 후진 경로가 자동 검증됐다고 표시하지 않습니다. 충전기 입지의 후보에서도 같은 경로를 확인할 수 있습니다. 후보는 설치된 EV 시설과 구분합니다.

검사는 차체와 여유 폭을 포함하며 벽·기둥·승강기·계단·문의 직사각형 교차와 위험 원형 영역을 검사합니다. 경로의 검사 간격은 최대 12cm, 차로 경계의 차체 표본 간격은 최대 30cm입니다. 도면 축척·연결은 미실측 상태이고 현장 승인이나 정밀 차량 주행 해석을 대체하지 않습니다.
...
원본 1536×822px의 X1–X12 치수 합계 92.75m(1130px)로 축척을 보정했습니다. 주차선·치수선·하부 공간 투영선을 제외하고 벽·난간 48개와 코어 출입 틈을 주석했습니다. PS 2곳은 설비 샤프트로 바로잡고 남측 승강기 2곳·계단 4곳·기둥 14개·중앙 OPEN 표시를 합쳐 오브젝트 23개를 제공합니다. 벽 높이 2.8m와 난간 1.1m는 표현 가정입니다. 도면 기재 87대와 실제 주석 31구획을 구분합니다. 높이가 다른 진입 램프·층간 연결, 현재 게이트 운영, 구획 내부 주차 조작은 아직 검증하지 않았습니다.
