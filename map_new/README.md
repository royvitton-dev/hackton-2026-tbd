...
**현재 범위:** 실제 장소 45곳·9개 유형·공개 도면 63장 + 별도의 합성 통합 시나리오, 원본 SVG/PNG/JPG/PDF와 해시, 3D 벽체 메시, 의미가 명시된 SVG의 Node/Graph 변환. 보관 도면은 구조선 후보와 한국어·영어 OCR 1,274개를 제공합니다. 원본을 검토한 4장에는 주차 구획 184면, 5장에는 기둥·계단·승강기·문·실·램프 134개를 추가했습니다. 문·안전 출구가 확인되지 않은 도면은 이동 경로를 생성하지 않습니다. 내 도면 가져오기는 SVG/분석 JSON/이미지/PDF 첫 페이지를 처리합니다.
...
**공간 표현·도면 정보**에서 바닥·벽·기둥·계단에 콘크리트, 도장, 벽돌, 타일, 에폭시, 아스팔트 재질을 각각 적용합니다. 계단 단·난간, 기둥 안전 띠, 승강기 문, 차량 차체·유리·바퀴·등을 입체로 표시합니다. 높이·단수·재질은 미리보기 가정이고 실측 BIM은 아닙니다. 차량 표시와 OCR 글자는 켜고 끌 수 있습니다. 장애인 구획은 파란 바닥·흰 휠체어·파란 안내판으로 표시하며 차량이 표식을 가리지 않습니다.
