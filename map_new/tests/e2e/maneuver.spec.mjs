...
async function open(page,site){await page.goto(`/map_new/?site=${site}`);await page.waitForFunction(id=>window.__parking?.state.plan?.id===id,site);await expect(page.locator('#world')).toHaveAttribute('data-ready','true');await page.evaluate(()=>document.fonts.ready);}
test('reverses into an actual Daecheon bay, pauses for shifts and finishes with the nose facing the aisle',async({page})=>{
 await open(page,'parking-131601-0');await page.locator('#parking-mode').selectOption('reverse');await expect(page.locator('#play')).toBeEnabled();await expect(page.locator('#destination')).toHaveValue('parking:north-8');await expect(page.locator('#route-message')).toContainText('후진 입차');
