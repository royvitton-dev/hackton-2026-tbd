...
export function VehicleSelector({users,selected,onChange}:{users:UserVehicle[];selected:UserVehicle;onChange:(id:string)=>void}){
  ...
  return <section className="selector-panel" aria-label="사용자 및 차량 선택">
    ...
    <div className="selector-fields">
      <label className="search-field"><span>사용자 · 차량 검색</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="사용자 ID 또는 모델명"/></label>
      <label className="user-field"><span>사용자 / 연결된 차량</span><select aria-label="사용자 및 차량" value={selected.userId} onChange={e=>onChange(e.target.value)}>
        {!options.some(u=>u.userId===selected.userId)&&<option value={selected.userId}>{selected.userId} · {selected.vehicle.model} (현재 선택)</option>}
        {options.map(u=><option key={u.userId} value={u.userId}>{u.userId} · {u.vehicle.manufacturer} {u.vehicle.model} · {u.vehicle.trim}</option>)}
      </select></label>
