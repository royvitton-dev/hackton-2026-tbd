...
export function GarageEnvironment(){
  return <group>
    ...
    <mesh rotation={[-Math.PI/2,0,0]} position={[0,-.20,0]}><planeGeometry args={[80,80]}/><meshStandardMaterial color="#050c16" roughness={.85} metalness={.05}/></mesh>
    <mesh position={[0,-.12,0]} receiveShadow><cylinderGeometry args={[3.05,3.12,.16,128]}/><meshStandardMaterial color="#101e32" metalness={.4} roughness={.42}/></mesh>
    ...
    <mesh position={[0,3,-8]}><boxGeometry args={[30,6,.2]}/><meshBasicMaterial color="#050c17"/></mesh>
    {[-6,-3,0,3,6].map(x=><mesh key={x} position={[x,2.5,-7.85]}><boxGeometry args={[.01,5,.02]}/><meshBasicMaterial color="#0a1c2e"/></mesh>)}
