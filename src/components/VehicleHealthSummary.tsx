...
export function VehicleHealthSummary({user,focused,onFocus}:{user:UserVehicle;focused:boolean;onFocus:()=>void}){
  ...
  return <aside className="health-summary" aria-label="Battery Health 요약">
    <h2>25°C Reference Stress Score</h2>
    <div className={`score-gauge ${score!==null&&score<60?'warning':''}`}>
      ...
      <div><strong data-testid="health-score">{score??'—'}</strong><span>/ 100</span><small>논문 모델 상대 점수</small></div>
    </div>
    ...
    <p className="measurement-note">Schmalstieg–Ecker NMC111 모델의 25°C 기준 상대 점수입니다.<br/>실제 SOH·잔존수명 예측값이 아닙니다.</p>
  </aside>;
}
