...
test('course variety: one or two goals per map, five Cloud ramps, Garden timing gate, Candy side bumpers',()=>{
 ...
 assert.equal(garden.rotors.filter(r=>r.ride==='flower-gate').length,2);assert.deepEqual(MAPS.map(m=>m.exits.length),[1,2,1,2]);assert.deepEqual(MAPS.map(m=>m.exits[0].x),[310,240,310,240]);assert.ok(candy.bumpers.some(b=>b.x===104&&b.y===1995));assert.ok(candy.bumpers.some(b=>b.x===516&&b.y===1995));
});
test('Garden final flower gate physically blocks and opens with its rotation',()=>{
 ...
 function contact(phase){const r=new Race({...makeConfig('A',1,'first',1),mapId:'split'},8);r.map=structuredClone(map);r.map.rotors=[{...gate,phase,omega:0}];r.rotationTime=0;const b=r.balls[0];Object.assign(b,{x:240,y:2094,vx:0,vy:250});r.segment(b,r.rotorSegments()[0],.65);return b;}
