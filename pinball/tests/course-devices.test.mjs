...
test('course variety: one Starlight goal, five Cloud ramps, four Garden timing gates, Candy side bumpers',()=>{
 ...
 assert.equal(garden.rotors.filter(r=>r.ride==='flower-gate').length,4);assert.ok(candy.bumpers.some(b=>b.x===104&&b.y===1995));assert.ok(candy.bumpers.some(b=>b.x===516&&b.y===1995));
});
test('Garden final flower gate physically blocks and opens with its rotation',()=>{
 ...
 function contact(phase){const r=new Race({...makeConfig('A',1,'first',1),mapId:'split'},8);r.map=structuredClone(map);r.map.rotors=[{...gate,phase,omega:0}];r.rotationTime=0;const b=r.balls[0];Object.assign(b,{x:100,y:2094,vx:0,vy:250});r.segment(b,r.rotorSegments()[0],.65);return b;}
...
test('all new rides activate in natural seeded races and finish with contained unique arrivals',async()=>{
 ...
 for(const map of MAPS){const counts={magnet:0,cannon:0};for(let seed=1;seed<=10;seed++){
  const r=new Race({...makeConfig(Array.from({length:20},(_,i)=>`R${i}`).join('\n'),1,'last',1),mapId:map.id,boardMotion:true},seed);r.start();let captures=0,launches=0;
  while(!['complete','invalid'].includes(r.state)){r.step();for(const e of r.events.splice(0)){if(e.type==='capture'){counts[e.kind]++;captures++;assert.ok(e.duration>=.5&&e.duration<=1);}if(e.type==='launch')launches++;}for(const b of r.balls){assert.ok(b.x>=40-1e-5&&b.x<=580+1e-5);assert.ok(b.y<=map.finish+1e-5);}}
 ...
 await writeFile('evidence/park-20260921/09-course-device-results.json',JSON.stringify({status:'PASS',cases:report},null,2));
});
