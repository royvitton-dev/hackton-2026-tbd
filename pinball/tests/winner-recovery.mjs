...
try{
 for(const mobile of [false,true]){
  ...
  await p.locator('.board-wrap').screenshot({path:`evidence/park-20260921/05-${device}-large-winner.png`});
 ...
 report.status='PASS';
}catch(e){report.status='FAIL';report.error=e.stack;process.exitCode=1;console.error(e);}finally{await writeFile('evidence/park-20260921/05-winner-recovery-results.json',JSON.stringify(report,null,2));await browser.close();}
