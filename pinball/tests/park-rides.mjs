...
try{
 ...
 for(const map of ['neon','orbit','zigzag','split']){
  ...
  await p.locator('#view').click();await p.locator('#board').screenshot({path:`evidence/park-20260921/02-${map}-overview.png`});await p.locator('#view').click();
  await p.locator('#start').click();await p.waitForFunction(()=>document.body.dataset.state==='racing');await p.waitForTimeout(2200);await p.locator('#board').screenshot({path:`evidence/park-20260921/02-${map}-live.png`});
...
}catch(e){report.status='FAIL';report.error=e.stack;process.exitCode=1;console.error(e);}finally{await writeFile('evidence/park-20260921/02-rides-results.json',JSON.stringify(report,null,2));await browser.close();}
