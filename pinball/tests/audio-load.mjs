...
try{
 for(const mobile of [false,true]){
  ...
  assert.equal(await page.locator('#sound').getAttribute('aria-pressed'),'false');await page.locator('#view').click();assert.equal(await page.locator('#arena-sound').isVisible(),true);if(mobile)await page.setViewportSize({width:320,height:700});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.screenshot({path:out+'/' +(mobile?'mobile':'desktop')+'-expanded.png'});
...
finally{await browser.close();await writeFile(out+'/load.json',JSON.stringify(report,null,2));}
