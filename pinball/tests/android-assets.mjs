...
try{
 ...
 for(const mode of ['player','lotto']){
  ...
  await page.waitForFunction(()=>['complete','invalid'].includes(window.pinball.snapshot().state),null,{timeout:70000});const r=await page.evaluate(()=>window.pinball.exportRound());assert.equal(r.result.state,'complete');assert.equal(await page.evaluate(()=>window.pinball.screenAwake().held),false);assert.equal(r.result.finishOrder.length,mode==='lotto'?7:1);assert.equal(await page.locator('#winner-name').textContent(),mode==='lotto'?'행운의 번호':r.result.winner.label);
  ...
  await page.locator('#splash-replay').click();assert.equal((await page.evaluate(()=>window.pinball.snapshot())).state,'mixing');await page.locator('#reset').click();assert.equal((await page.evaluate(()=>window.pinball.snapshot())).state,'ready');await page.locator('#focus-mode').click();report.tests.push({mode,status:'PASS',arrivals:r.result.finishOrder.length});console.log('PASS APK assets',mode);
 }
