...
try{
 for(const mobile of [false,true]){
  ...
  await p.locator('#participants').fill('하늘,하늘,구름,별,솜사탕,우주선');await p.locator('[data-speed="3"]').click();
  for(const rule of ['first','nth','last']){
   await p.locator(`input[name=rule][value=${rule}]`).check();if(rule==='nth')await p.locator('#nth').fill('3');
  ...
  await p.locator('input[name=rule][value=last]').check();await p.locator('#participants').fill('가나다라마바사아자차'.repeat(4));await p.locator('#start').click();await p.waitForFunction(()=>document.body.dataset.state==='complete',null,{timeout:8000});const single=await p.evaluate(()=>window.pinball.snapshot());assertGameResult(single);assert.equal(single.finishOrder.length,0);assert.equal(single.winner.label.length,40);
