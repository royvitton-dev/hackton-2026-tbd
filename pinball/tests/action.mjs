...
try{
 for(const mobile of [false,true]){
  ...
  await p.locator('#board').screenshot({path:`evidence/park-20260921/03-action-${device}-live.png`});
  ...
  await writeFile(`evidence/park-20260921/03-action-${device}-round.json`,JSON.stringify(round,null,2));
 ...
 report.status='PASS';
}catch(e){report.status='FAIL';report.error=e.stack;process.exitCode=1;console.error(e);}finally{await writeFile('evidence/park-20260921/03-action-browser-results.json',JSON.stringify(report,null,2));await browser.close();}
