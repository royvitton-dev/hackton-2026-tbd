...
for(const mobile of [false,true]){
 ...
 for(const label of ['측면','상단','입체']){await p.locator('.camera-angle').click();assert.match(await p.locator('.camera-angle').textContent(),new RegExp(label));await p.waitForTimeout(80);assert.deepEqual(await p.evaluate(()=>window.pinball.snapshot()),paused);await p.locator('#board').screenshot({path:`evidence/3d-${mobile?'mobile':'desktop'}-${label}.png`});}
