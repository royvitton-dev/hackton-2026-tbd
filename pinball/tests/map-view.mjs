...
try{for(const [width,height,fallback] of [[1440,1000,false],[390,844,false],[320,700,false],[844,390,false],[390,844,true]]){
 ...
 await p.locator('#participants').fill('별빛\n구름\n바이킹\n솜사탕\n우주선\n찻잔');await p.locator('[data-speed="3"]').click();await p.locator('#start').click();await p.waitForFunction(()=>window.pinball.snapshot().state==='racing');await p.locator('#pause').click();const frozen=await p.evaluate(()=>window.pinball.snapshot());
