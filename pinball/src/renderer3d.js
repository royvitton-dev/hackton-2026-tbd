...
export class Renderer3D {
 constructor(canvas){
  this.canvas=canvas;this.mode='webgl';this.overview=false;this.lastRound=null;this.cameraCenter=400;this.finishedAt=new Map();
  ...
  this.scene=new THREE.Scene();this.camera=new THREE.OrthographicCamera(-6,6,8,-8,.1,110);
  ...
  this.progress=document.createElement('div');this.progress.className='course-progress';canvas.parentElement.append(this.progress);this.observer=new ResizeObserver(()=>this.resize());this.observer.observe(canvas);this.resize();
 ...
 build(race){
  ...
  const board=this.mesh(new THREE.ExtrudeGeometry(shape,{depth:.35,bevelEnabled:true,bevelSegments:2,steps:1,bevelSize:.035,bevelThickness:.035}),this.material(theme.floor,.16,.42),0,-.36,0);board.rotation.x=-Math.PI/2;board.material.envMapIntensity=.12;
 ...
 draw(race,reduced=false,selected=null){
  if(this.lastRound!==race.roundId)this.build(race);const map=race.map,aspect=this.aspect||1;const halfW=5.95,halfH=halfW/aspect;const span=halfH*2/.857*60;
  const active=race.balls.filter(b=>!b.finished).sort((a,b)=>a.y-b.y);const progress=active[Math.floor(active.length*.65)]?.y??map.finish;const target=['ready','mixing','countdown'].includes(race.state)?span/2-60:Math.max(span/2-60,Math.min(map.height-span/2+30,progress+100));
  if(race.state!=='paused')this.cameraCenter=reduced?target:this.cameraCenter+(target-this.cameraCenter)*.08;
  const center=this.overview?map.height/2:this.cameraCenter;const h=this.overview?map.height*S*.46:halfH;this.camera.left=-h*aspect;this.camera.right=h*aspect;this.camera.top=h;this.camera.bottom=-h;this.camera.updateProjectionMatrix();this.camera.position.set(.22,16,Z(center)+9);this.camera.lookAt(0,0,Z(center));
  ...
  this.hud.textContent=this.overview?'COURSE OVERVIEW':`SECTOR ${Math.min(3,Math.floor((center-200)/750)+1).toString().padStart(2,'0')} / 03`;
