...
function updateCatalog(next){
 ...
 $('#sync-label').textContent=state.hosting==='sites'?'온라인 파크 운영 중':state.watcher?(state.sync?.remote==='pending'?'새 커밋 확인 · 반영 대기':state.sync?.remote==='offline'?'로컬 운영 · 원격 재연결 대기':'10분마다 파크 업데이트'):'로컬 파크 운영 중';
...
const events=state.hosting==='sites'?null:new EventSource('/api/events');if(events){events.onmessage=e=>{try{updateCatalog(JSON.parse(e.data));}catch{}};events.onerror=()=>{connectionError=true;$('#sync-label').textContent='파크 연결 복구 중';};}
