import { sphere, cone, cyl, group, torus, staticBatch } from './materials.js';
import { buildFriend } from './character-friends.js';
export function disneyCharacter(parent,kind='mickey',scale=1){
 const root=group(parent);root.name=`Disney character: ${kind}`;root.scale.setScalar(scale);root.userData.dynamic=true;root.userData.character=kind;
 const black='#26282b',white='#fff7e7',skin='#f2d1aa',yellow='#e5b440',red=kind==='minnie'?'#d95979':'#cb4640';
 const body=group(root);const arms=[],details=[];
 if(kind==='olaf'){
  ...
  sphere(body,.29,.39,.25,daisy?'#a886bc':'#416d9b',0,.75,0);sphere(body,.31,.34,.28,white,0,1.29,0);
  ...
  if(daisy){
   for(const side of [-1,1]){
    sphere(body,.17,.115,.065,'#dc7eae',side*.14,1.62,.1);
    for(let i=0;i<3;i++){const lash=cyl(body,.012,.08,black,side*(.07+i*.035),1.51,.28);lash.rotation.z=-side*(.15+i*.22);}
   }
   sphere(body,.065,.07,.055,'#c55d97',0,1.62,.15);
  }else{cyl(body,.23,.08,'#326591',0,1.59,0);sphere(body,.22,.075,.2,'#5787b0',0,1.66,0);}
  for(const side of [-1,1]){sphere(body,.15,.065,.24,daisy?'#d87caa':yellow,side*.16,.12,.1);cyl(body,.065,.25,yellow,side*.13,.3,0);const a=group(body,side*.26,.95,0);sphere(a,.085,.22,.085,white,side*.08,-.1,0);if(daisy){sphere(a,.1,.1,.1,'#a886bc',side*.025,.025,0);const bracelet=torus(a,.085,.018,'#dab655',side*.08,-.23,0);bracelet.rotation.x=Math.PI/2;}arms.push(a);}
  if(!daisy)for(const side of [-1,1]){const b=cone(body,.14,.18,'#c94c47',side*.1,.96,.26);b.rotation.z=side*Math.PI/2;}
 ...
 for(const arm of arms){arm.userData.dynamic=true;staticBatch(arm);}
 for(const {part} of details){part.userData.dynamic=true;staticBatch(part);}staticBatch(body);staticBatch(base);
