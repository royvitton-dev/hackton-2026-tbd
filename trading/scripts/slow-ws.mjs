// Bounded process-level slow-receiver probe; never controls the running demo.
...
assert.deepEqual(process.argv.slice(2), ['--competing-resource-stress'], 'Usage: node scripts/slow-ws.mjs --competing-resource-stress');
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const started = performance.now();
const runId = `${new Date().toISOString().replace(/[:.]/g, '-')}-slow-ws-${crypto.randomUUID().slice(0, 8)}`;
...
const metadata = { run_id: runId, started_at: new Date().toISOString(), command: process.argv, classification: 'competing-resource slow-WebSocket validation while normal demo/observer remain live; not quiet latency baseline', limits, workload: '384 one-hour resting buys at700..702points split over12botaccounts;128seedbeforeWS,256duringpausedread;32owner cancellations; all416 durable results looked up and16 duplicate-retried. Two simultaneous WS clients, followed by one replacement.', binary: { source, copied: executable, sha256: sha(executable) }, source_sha256: { main: sha(path.join(root, 'engine', 'src', 'main.rs')), script: sha(fileURLToPath(import.meta.url)) }, host: { node: process.version, os: os.version(), cpu: os.cpus()[0]?.model, processors: os.cpus().length }, demo_before: demo() };
...
let engine, sampler, base, normal, replacement, slow, failure, state, httpCount = 0, active = 0, maxActive = 0, cleanup = false;
...
try {
  ...
  slowObservation = { remote_end_observed: slow.remote_end, closed_before_client_cleanup: slow.closed && !slow.local_destroy, close_frame: slow.close_frame ?? null, last_complete_event_seq: lastSlowSeq, final_event_seq: state.event_seq, pending_partial_frame_bytes: slow.pending.length, cause: 'Production server does not log WS close reasons; broadcast lag versus send timeout cannot be uniquely attributed from this client trace.' };
  ...
  console.log(JSON.stringify(summary)); console.log(`Slow WS evidence: ${directory}`);
}
