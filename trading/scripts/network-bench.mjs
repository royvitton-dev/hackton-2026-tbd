...
const help = `Usage: node scripts/network-bench.mjs --quiet-window [--cycles 200] [--warmup-cycles 20] [--stress12] [--stress-rounds 30] [--label baseline] [--transport fetch|node-http]
...
const metadata = {
  ...
  options, quiet_window_operator_confirmed: true, seed: 20260921, seed_semantics: 'deterministic fixed workload; no claim about reproducing concurrent process scheduling',
...
async function startEngine(directory) {
  ...
  const stderr = fs.openSync(path.join(directory, 'engine.stderr.log'), 'wx');
  const child = spawn(executable, [], { cwd: path.join(root, 'engine'), windowsHide: true,
    env: { ...process.env, ENGINE_BIND: `127.0.0.1:${port}`, ENGINE_DATA_DIR: path.join(directory, 'data'), ALLOWED_ORIGINS: 'http://127.0.0.1:5175' }, stdio: ['ignore', stdout, stderr] });
  fs.closeSync(stdout); fs.closeSync(stderr);
  let spawnError;
  child.on('error', error => { spawnError = error; });
  child.on('exit', (code, signal) => emit({ event: 'engine_exit', phase_dir: directory, pid: child.pid, code, signal }));
  emit({ event: 'engine_started', phase_dir: directory, pid: child.pid, port });
  const engine = { child, base, port, directory,
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      if (!running(child)) throw new Error(`Engine exited before readiness: ${child.exitCode}/${child.signalCode}`);
      try {
        const response = await fetch(base + '/health', { signal: AbortSignal.timeout(500) });
        if (response.ok) { engine.health = await response.json(); return engine; }
      } catch { /* Startup only, outside measurement. */ }
      await sleep(100);
    }
    throw new Error('Isolated engine readiness timed out');
  } catch (error) { if (running(child)) child.kill('SIGKILL'); throw error; }
}
async function stopEngine(engine) {
  if (!engine || !running(engine.child)) return { method: 'already_exited' };
  let shutdown;
  let actualConfig;
  try { shutdown = await jsonRequest(engine.base, '/api/admin/shutdown', { method: 'POST' }); }
  catch (error) { shutdown = { error: error.message }; }
  const deadline = Date.now() + 15_000;
  while (running(engine.child) && Date.now() < deadline) await sleep(50);
  const forced = running(engine.child);
  if (forced) engine.child.kill('SIGKILL');
  return { method: forced ? 'forced_after_shutdown_timeout' : 'admin_shutdown', response: shutdown, exit_code: engine.child.exitCode, signal: engine.child.signalCode };
}
...
function startMemorySampler(engine, directory) {
  ...
  if (process.platform !== 'win32') {
    ...
    return async () => { clearInterval(timer); sample(); };
  }
...
`;
  ...
  sampler.on('error', error => emit({ event: 'memory_sampler_error', error: error.message, directory }));
  ...
  return async () => {
    ...
    const deadline = Date.now() + options.memoryIntervalMs + 2_000;
    while (running(sampler) && Date.now() < deadline) await sleep(50);
    if (running(sampler)) sampler.kill('SIGKILL');
  };
}
...
async function measureScenario(kind) {
  ...
  try {
    ...
    emit({ event: 'scenario_failed', scenario: kind, error: error.message });
  } finally {
    if (stream) stream.close();
    engine?.httpAgent?.destroy();
    if (stopSampler) await stopSampler();
    ...
    if (engine) shutdown = await stopEngine(engine);
  }
  ...
  const summary = { ...summarize(rows, elapsedMs, expected, kind), started_at: scenarioStarted, ended_at: new Date().toISOString(),
    ...
    durability: engine?.health.durability ?? null, shutdown,
  ...
  if (phaseError) { summary.complete = false; if ('met' in summary.target) summary.target.met = false; }
...
try {
  for (const kind of options.stress12 ? ['B', 'C', 'stress12'] : ['B', 'C']) {
    ...
    summaries.push(await measureScenario(kind));
  }
} catch (error) {
