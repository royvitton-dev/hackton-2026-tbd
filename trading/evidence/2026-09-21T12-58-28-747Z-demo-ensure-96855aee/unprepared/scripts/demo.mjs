...
import { demoEndpoints, reserveLoopbackPorts, monitorChild, engineReadyLog, frontendReadyLog, waitForOwnedReady, acquireLauncherLock, ensureDecision, GRACEFUL_EXIT_MS } from './demo-lifecycle.mjs';
...
async function restoreFrontend(manifest) {
  ...
  try {
    ...
    const record = { started_at: new Date().toISOString(), previous_frontend: old ?? null, new_pid: child.pid, evidence: replacementDir, status: 'starting' };
    ...
    try {
      ...
      Object.assign(record, { status: 'failed', error: error.message }); writeJson(path.join(replacementDir, 'replacement.json'), record);
