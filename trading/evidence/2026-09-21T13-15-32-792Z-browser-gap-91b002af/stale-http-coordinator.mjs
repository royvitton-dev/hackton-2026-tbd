...
const record = (event, data = {}) => {
  ...
  fs.appendFileSync(path.join(directory, 'stale-http-coordinator.jsonl'), JSON.stringify(row) + '\n');
};
...
const marker = path.join(directory, 'browser-seq3-before-release.json');
...
try {
  ...
  record('captured', { hold: captured });
  const capturedObservedAt = Date.now();
  const request = { request_id: randomUUID(), action: { type: 'place', side: 'buy', price: 800, quantity: 1 } };
  ...
  assert.ok(observedBeforeRelease, 'Browser did not confirm seq3 before the six-second release guard');
