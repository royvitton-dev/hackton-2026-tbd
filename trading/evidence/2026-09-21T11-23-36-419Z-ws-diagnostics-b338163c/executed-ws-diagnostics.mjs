...
let normalState, normalClosed = false, slowObservation;
...
function connectNormal(label, onState) {
  ...
  assert.equal(websocketClients[label], undefined, 'Each WebSocket diagnostic has a unique label');
  const client = websocketClients[label] = { label, events: [], errors: [], close: null };
  const socket = new WebSocket(base.replace('http:', 'ws:') + '/ws');
  socket.addEventListener('message', message => {
    try {
      const payload = JSON.parse(message.data); assert.equal(payload.type, 'state');
      const received = { label, at_ms: performance.now() - started, event_seq: payload.state.event_seq, bytes: Buffer.byteLength(message.data) };
      normalEvents.push(received); client.events.push({ event: 'state', ...received });
      onState(payload.state);
    } catch (error) {
      const failure = { event: 'message_error', at_ms: performance.now() - started, message: error.message, cleanup };
      client.errors.push(failure); client.events.push(failure); stop.abort(error);
    }
  });
  socket.addEventListener('close', close => {
    client.close = { event: 'close', at_ms: performance.now() - started, code: close.code, reason: close.reason, cleanup };
    client.events.push(client.close);
    event({ event: `${label}_close`, code: close.code, reason: close.reason, cleanup });
    if (label === 'normal') normalClosed = true;
  });
  socket.addEventListener('error', () => {
    const failure = { event: 'error', at_ms: performance.now() - started, message: `${label} error`, cleanup };
    client.errors.push(failure); client.events.push(failure);
  });
  return socket;
}
...
try {
  ...
  await until(() => websocketClients['peer-close-case'].close !== null, 3000, 'Peer receives close acknowledgement');
  assert.equal(websocketClients['peer-close-case'].close.code, 1000, 'Peer close handshake preserves normal code 1000');
  assert.deepEqual(websocketClients['peer-close-case'].errors, [], 'Peer close handshake has no client errors');
  assert.equal(diagnosticCases.peer.close_reply, 'flushed', 'Server flushes the queued close acknowledgement');
  ...
  assert.deepEqual(normalState, state); assert.equal(normalClosed, false); assert.deepEqual(websocketClients.normal.errors, []);
  ...
  assert.deepEqual(replacementState, state); assert.deepEqual((await request('/api/state')).body, state); assert.equal(normalClosed, false); assert.deepEqual(websocketClients.normal.errors, []); assert.deepEqual(websocketClients.replacement.errors, []);
  ...
  save('websocket-clients.json', websocketClients);
