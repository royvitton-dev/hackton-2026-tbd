import { resourceProcessIdentity, sameResourceProcessIdentity } from './resource-memory.mjs';
...
export function resourceCpu(samples, logicalProcessors, expectedProcessCount) {
  ...
  const identitySample = resources.find(sample => resourceProcessIdentity(sample.resources, expectedProcessCount) !== null);
  const identity = identitySample ? resourceProcessIdentity(identitySample.resources, expectedProcessCount) : null;
  ...
  for (let index = 1; index < resources.length; index++) {
    ...
    if (!reason && (!sameResourceProcessIdentity(before.resources, identity) || !sameResourceProcessIdentity(after.resources, identity))) reason = 'process_set_changed';
    const processes = [];
    if (!reason) {
  ...
  return {
    ...
    process_identity_baseline: identitySample ? { at: identitySample.at, processes: identity, basis: 'first_complete_resource_membership' } : null,
    ...
    limitations: [
      ...
      'PID/name membership is pinned to the first complete resource reading. Historical samples without start times cannot exclude reuse with the same PID and process name.',
