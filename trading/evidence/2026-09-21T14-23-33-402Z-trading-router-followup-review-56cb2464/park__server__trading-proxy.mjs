...
export function createTradingProxy(endpoint='http://127.0.0.1:8787',{timeout=10000}={}){
 ...
 return {
  request(req,res){
   if(!allowedGatewayRequest(req)){res.writeHead(403,{'Content-Type':'application/json; charset=utf-8'});res.end(JSON.stringify({error:{code:'ORIGIN_DENIED',message:'거래소는 현재 로컬 서버와 같은 출처에서만 연결할 수 있습니다.'}}));return;}
  ...
  upgrade(req,socket,head){
   if(!allowedGatewayRequest(req)){socket.end('HTTP/1.1 403 Forbidden\r\nConnection: close\r\n\r\n');return;}
