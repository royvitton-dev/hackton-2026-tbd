...
const packages = [
  ...
  { name: 'crossbeam-utils', version: '0.8.23', checksum: 'a31eee39dddec8330830986fcd7625edb5a24ec90ea038215273bbc3adb08ac6', files: ['Cargo.toml', 'src/backoff.rs', 'src/primitive.rs'] },
].map(pkg => {
  const prefix = `${registry}/${pkg.name}-${pkg.version}`;
  const manifest = JSON.parse(read(`${prefix}/.cargo-checksum.json`));
  ...
  checked(manifest.package === pkg.checksum, `Registry package checksum differs: ${pkg.name}`);
  ...
  const files = pkg.files.map(p => {
    ...
    checked(entry.sha256 === manifest.files[p], `Cached source checksum differs: ${p}`);
    return { ...entry, verified_against_cargo_checksum: true };
  });
...
const pinnedExpected = {
  ...
  'engine/target/release/trading-engine.exe': '65348c203d3ddf0a29cb3fa19eca650347e4cd77308f2352f434212063896512',
