...
Wonder Park에는 `trading/attraction.json`으로 거래소를 등록했다. 현재 통합 서버의 입장 API는 같은 출처의 `/trading/`를 반환하고 HTTP·WebSocket은 `/trading/backend/`로 전달한다. 파크 시작과 거래소 입장 시 준비 함수가 `demo.mjs ensure`로 기본 로컬 UI·엔진·봇을 준비하거나 기존 실행을 재사용한다. 최초 의존성 준비는 별도로 필요하다. [통합 서버 설정](../../park/ROUTER.md)을 따른다.

독립 Vercel UI는 위 절차대로 실제 HTTPS UI 주소와 별도 HTTPS/WSS 엔진 주소를 구성한다. 현재 Park 입장 API는 앱별 경로를 사용하므로 `attraction.json`의 URL만 바꿔도 외부 Vercel 화면으로 이동하는 것은 아니다. 공개 Park 연결은 별도 라우팅·인증·Origin 검토가 필요하며 이번 로컬 연동 검증에 포함되지 않는다. 다른 호스트용 선택적 React 어댑터와 실제 검증 범위는 [UI 문서](ui.md)에 있다.
