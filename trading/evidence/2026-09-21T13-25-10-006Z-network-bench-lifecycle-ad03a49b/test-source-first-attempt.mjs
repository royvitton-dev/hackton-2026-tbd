...
test('benchmark lifecycle corrections without engine workloads or live service control', { timeout: 25000 }, async t => {
  try {
    ...
    await t.test('owned matching ready marker gates a short-lived HTTP fixture', async () => {
      const instance = start('owned-ready', `import http from 'node:http';const server=http.createServer((q,r)=>{r.end('ok');if(q.url==='/stop')server.close(()=>process.exit(0));});server.listen(0,'127.0.0.1',()=>console.error(JSON.stringify({event:'ready',bind:'127.0.0.1:'+server.address().port,data_dir:${JSON.stringify(runDir)}})));`);
      try {
        let marker;
        ...
        assert.equal((await fetch(`http://${marker.bind}/stop`)).status, 200);
