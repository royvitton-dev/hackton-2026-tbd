...
fn historical_snapshot_reconstructed_from_tracked_json_preserves_transitions() {
    ...
    // Leave the historical snapshot at sequence 4. A restart must replay the
    // nine durable suffix records, not merely deserialize a fresh checkpoint.
    drop(store);
    let mut replayed = Store::open(&directory, Config::default()).unwrap();
    let suffix_recovery = replayed.recovery_report().clone();
    assert!(suffix_recovery.snapshot_used.is_some());
    assert_eq!(suffix_recovery.journal_records, history.len() as u64);
    assert_eq!(
        suffix_recovery.replayed_records,
        (history.len() - prefix) as u64
    );
    assert_eq!(
        serde_json::to_value(replayed.core()).unwrap(),
        history.last().unwrap().full_core
    );
    let mut checked_requests = BTreeSet::new();
    for entry in &history {
        if !checked_requests.insert((&entry.command.account_id, &entry.command.request_id)) {
            continue;
        }
        assert_eq!(
            replayed
                .core()
                .lookup(&entry.command.account_id, &entry.command.request_id)
                .unwrap(),
            entry.result
        );
    }
    replayed.core().check_invariants().unwrap();
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&serde_json::json!({
            ...
            "journal_suffix_recovery": suffix_recovery, "restored_request_results_equal": true,
            "distinct_restored_requests": checked_requests.len(),
