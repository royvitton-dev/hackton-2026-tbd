...
`;
  ...
  const stop = async () => {
    const stoppedEarly = monitor.state.exited || !!monitor.state.spawn_error;
    let stopError;
    ...
    return { ...exit, method: 'stop_file', stopped_before_request: stoppedEarly, stop_error: stopError, success: exit.success && !stopError && !stoppedEarly };
  };
  try {
