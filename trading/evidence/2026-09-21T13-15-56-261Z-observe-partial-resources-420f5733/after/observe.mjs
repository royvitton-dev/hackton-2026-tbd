...
function resourceStats() {
  if (!manifest) return { resources: { limitation: 'No demo process manifest' } };
  const pids = manifest.processes.map(p => Number(p.pid)).filter(Number.isInteger);
  try {
    if (process.platform === 'win32') {
      return collectWindowsResources(manifest.processes);
    }
    return { resources: pids.map(pid => ({ pid, status: fs.readFileSync(`/proc/${pid}/status`, 'utf8') })) };
  } catch (error) { return { resources: { error: error.message } }; }
}
...
try {
  while (performance.now() - startedMonotonic <= seconds * 1000) {
    ...
    if (Date.now() - lastResources > 30000) {
      Object.assign(sample, resourceStats());
