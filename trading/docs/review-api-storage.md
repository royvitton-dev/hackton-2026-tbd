...
Follow-up 2026-09-21 10:21 UTC: the previously source-reviewed slow-WebSocket isolation path now has a bounded actual-process run. A paused raw receiver coexisted with 288 new durable commands and a normal receiver that received all 289 consecutive revisions; all 416 original results were looked up, assets/reserves matched independent expectations, and reconnect recovered the final state. The paused peer later observed `ECONNRESET` before client cleanup, but current production logs cannot distinguish broadcast lag from a send deadline. See [slow receiver validation](slow-ws-validation.md) for exact limits, evidence and cleanup; this does not add a handshake/shutdown-overlap test or a quiet performance baseline.
