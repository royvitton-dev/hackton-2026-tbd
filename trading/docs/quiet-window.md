...
Rust 실행 파일을 사용하는 새 PowerShell마다 먼저 `. ./scripts/env.ps1`를 적용한다. 이는 이미 설치된 도구·DLL 경로와 Rust 메타데이터를 동일하게 맞추는 단계이며 재빌드 명령이 아니다. aged, A runner, B/C, 최종 demo ensure에도 같은 환경을 사용한다.
