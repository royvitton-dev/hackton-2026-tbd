...
export const browserMode = import.meta.env.VITE_TRADING_RUNTIME !== 'server'
...
const configuredWs =
  ...
  (routerMode
    ...
    : undefined)
export const API_URL = browserMode
  ? `browser:${window.location.origin}`
  : (configuredApi || (localHost ? 'http://127.0.0.1:8787' : '')).replace(/\/$/, '')
export const WS_URL = configuredWs || (localHost ? 'ws://127.0.0.1:8787/ws' : '')
export const configurationError = browserMode
  ? null
  : !API_URL || !WS_URL
...
export async function request(
  ...
  if (browserMode) return browserRequest(path, init)
