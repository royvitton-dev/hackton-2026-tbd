...
`node scripts/demo.mjs ensure`는 파크가 호출하는 재사용 가능한 시작 명령입니다. 실행 중인 시연을 재사용하거나, 실행이 없으면 시작하거나, UI만 종료돼 있으면 UI를 복원합니다. 엔진·봇 일부만 실행 중인 불완전한 시장은 자동으로 중복 실행하지 않고 상태 확인 안내를 반환합니다. UI만 복원하려면 `node scripts/demo.mjs restart-frontend`를 사용할 수도 있습니다. 파크 종료는 거래소를 자동 종료하지 않으며 전체 종료는 위 `stop` 명령을 사용합니다.
...
[장시간 CPU·메모리 그래프](evidence/20260921T135027159622Z-resource-coverage-plot-a553d498/resource-coverage.png)는 누락 구간을 그대로 표시합니다. [보완 분석](docs/resource-observation.md#pid-재사용-정정과-보완-수집-분석)은 프로세스 시작 시각을 대조하고, 기존 관찰의 다른 프로세스로 재사용된 PID 표본을 제외합니다. 22:17–22:45 구간의14개 합계 평균 CPU는0.8901%이며 장시간 수집은 진행 중입니다.
...
[WebSocket 종료 진단](docs/ws-diagnostics.md)은 정상 close 응답 수정과 느린 수신자의 실제 전송 시간 초과, 연결별 종료 로그를 검증합니다. 이전 고부하 단절의 원인과 서버 shutdown의 close 코드 한계는 별도로 명시합니다.

[WebSocket 직렬화 개선](docs/ws-serialization.md)은 같은 데이터를 전송하면서 중간 JSON 트리 생성을 없앴습니다. 실제 큰 상태에서 할당·재할당 15,261→12회와 회귀 검증을 기록했으며, 전송량·전체 서비스 성능과 구분합니다.

[주문 결과 문자열 할당 개선](docs/result-allocation.md)은 같은6,000개 신규 명령의 할당을14.331→8.331회/명령으로 줄였습니다. 이전·새 저장 데이터의 양방향 저널 재생, Rust48개 및 API10개 검사를 통과했습니다. 무할당 목표는 미달이며 처리량 개선은 별도 측정 대상입니다.
