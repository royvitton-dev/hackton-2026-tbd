...
test('project directory, shared paths, redirects and mobile golden',async({page,request})=>{
 ...
 for(const id of ['map','map_new','dopamin','pinball','trading','movie','voice','battery_health']){const response=await request.post('/api/launch?id='+id);expect(response.status()).toBe(200);const entry=new URL((await response.json()).url);expect(entry.pathname).toBe('/'+id+'/');expect(entry.search).toBe(id==='battery_health'?'?intro=pitstop':'');}
