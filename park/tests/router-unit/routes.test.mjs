...
import {APPS,NEXT_BASE_PATH,appPath,launchPath,matchApp,privatePath,staticPath,redirectPath} from '../../server/routes.mjs';
...
describe('one server routes',()=>{
 ...
 it('launches battery and vehicle entries through the pit stop without changing canonical routes',()=>{
  for(const id of ['battery_health','vehicle'])expect(launchPath(id)).toBe('/battery_health/?intro=pitstop');
  ...
  expect(directory().match(/href="\/battery_health\/\?intro=pitstop"/g)).toHaveLength(2);
 });
