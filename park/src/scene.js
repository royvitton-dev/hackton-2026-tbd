...
export class ParkScene {
 constructor(container,{onSelect,onReady,onError,onLabels,video,capture=false}={}){
  ...
  this.scene=new THREE.Scene();this.scene.background=new THREE.Color('#f2eee5');this.scene.fog=new THREE.Fog('#f2eee5',105,210);this.backgroundDirty=true;
  this.camera=new THREE.PerspectiveCamera(34,1,.1,300);this.camera.position.set(23,40,56);
  ...
  this.controls=new OrbitControls(this.camera,this.renderer.domElement);this.controls.target.set(0,2.2,0);this.controls.enableDamping=true;this.controls.dampingFactor=.07;this.controls.minDistance=22;this.controls.maxDistance=95;this.controls.maxPolarAngle=Math.PI*.43;this.controls.minPolarAngle=.22;this.controls.enablePan=true;this.controls.screenSpacePanning=false;
  this.controls.addEventListener('start',()=>{this.transition=null;this.tour=false;});
  this.sun=new THREE.DirectionalLight('#ffeacb',3.1);this.sun.position.set(-24,40,26);this.sun.castShadow=true;this.sun.shadow.mapSize.set(4096,4096);Object.assign(this.sun.shadow.camera,{left:-38,right:38,top:38,bottom:-38,near:.5,far:100});this.sun.shadow.bias=-.00012;this.sun.shadow.normalBias=.045;this.sun.shadow.radius=2;this.scene.add(this.sun);
  ...
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(600,600),new THREE.MeshStandardMaterial({color:'#f2eee5',roughness:1}));ground.rotation.x=-Math.PI/2;ground.position.y=-1.28;ground.receiveShadow=true;this.scene.add(ground);this.ground=ground;
  ...
  Promise.all([this.environmentReady,landscape.ready]).then(()=>this.renderer.compileAsync(this.scene,this.camera)).then(()=>{container.dataset.ready='true';onReady?.();}).catch(onError);
 }
 positions(index){return [[-13,.34,.8],[13,.34,2.2],[-13,.34,-11.8],[-12,.34,11.2],[12,.34,12]][index] || [Math.sin((index-5)*2.4+.4)*(25+Math.floor((index-5)/7)*9),.35,Math.cos((index-5)*2.4+.4)*(20+Math.floor((index-5)/7)*7)];}
 ...
 resize(){const {width,height}=this.container.getBoundingClientRect();if(!width||!height)return;this.camera.aspect=width/height;this.camera.fov=width<650?56:34;this.camera.updateProjectionMatrix();this.renderer.setSize(width,height);this.composer.setSize(width,height);this.ao.setSize(Math.round(width*.75),Math.round(height*.75));}
 frame(now){
  ...
  this.controls.update();this.renderer.info.reset();this.composer.render();
 ...
 overview(){this.tour=false;this.move([23,40,56],[0,2.2,0]);}
 ...
 setNight(night){this.night=night;const bg=night?'#192d36':'#f2eee5';this.scene.background=new THREE.Color(bg);this.scene.fog.color.set(bg);this.ground.material.color.set(bg);this.sun.intensity=night?.45:3.1;this.sun.color.set(night?'#b4cde8':'#ffeacb');this.hemisphere.intensity=night?.38:.65;this.bloom.strength=night?.4:.12;this.renderer.toneMappingExposure=night?1.1:.91;this.backgroundDirty=true;}
 ...
 leaveCinema(){this.video.pause();this.mode='park';this.parkRoot.visible=true;this.projects.visible=true;this.ground.visible=true;this.cinema.root.visible=false;this.scene.fog=new THREE.Fog('#f2eee5',105,210);this.scene.environmentIntensity=.55;this.controls.minDistance=22;this.controls.maxDistance=95;this.controls.maxPolarAngle=Math.PI*.43;this.controls.enablePan=true;this.setNight(this.night);this.overview();}
