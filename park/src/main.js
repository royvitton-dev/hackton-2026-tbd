...
<main class="park-page">
 <section class="intro" aria-label="파크 소개"><div class="eyebrow"><span></span> A LITTLE WORLD OF POSSIBILITIES</div><h1>Small world.<br>Endless <em>wonder.</em></h1><p>아이디어가 어트랙션이 되는 곳.<br>우리의 상상으로 자라나는 작은 테마파크.</p><button class="primary-button" id="tour">파크 산책하기 ${icon('arrow')}</button><div class="intro-foot"><span class="tiny-star">✧</span><p>Made of ideas.<br><strong>Built by all of us.</strong></p></div></section>
...
<section id="cinema-ui" class="cinema-ui" hidden><div class="cinema-top"><button id="leave-cinema" class="cinema-back">${icon('arrow')} 파크로 돌아가기</button><div><span>STARLIGHT CINEMA</span><h2>지금, 우리의 이야기</h2></div><span class="cinema-badge">PRIVATE SCREENING</span></div><div class="cinema-bottom"><div><small>NOW PLAYING</small><h3>VITALIS · 해커톤의 30초</h3><p>모든 가능성은 작은 아이디어에서 시작됩니다.</p></div><div class="film-controls"><button id="film-play" aria-label="영상 재생">${icon('play')}</button><span id="film-time">0:00 / 0:30</span><input id="film-seek" type="range" min="0" max="30" step=".1" value="0" aria-label="영상 재생 위치"><button id="film-mute" aria-label="영상 음소거">${icon('sound')}</button></div></div></section>
...
function select(id){
 ...
 const names={mickey:'미키 마우스',minnie:'미니 마우스',donald:'도널드 덕',olaf:'올라프'};
...
$('#settings').onclick=()=>{showModal(`<div class="eyebrow">MAKE YOURSELF AT HOME</div><h2>나만의 관람 환경</h2><p>파크의 작은 디테일까지 즐길 수 있도록 준비했습니다.</p><label class="setting-row"><span><strong>렌더링 품질</strong><small>그림자 · 반사 · 화면 공간 음영</small></span><select id="quality"><option value="ultra">최상</option><option value="high">높음</option><option value="balanced">부드러운 동작</option></select></label><label class="setting-row"><span><strong>움직임 줄이기</strong><small>캐릭터·어트랙션 모션을 정지합니다</small></span><input type="checkbox" id="reduce-motion" ${view?.reduced?'checked':''}></label><div class="settings-foot">드래그: 회전 · 스크롤: 확대/축소 · 우클릭 드래그: 이동<br>현재 렌더링 ${view?.fps||'—'} FPS · WebGL 2</div>`);$('#quality').value=currentQuality;$('#quality').onchange=e=>{currentQuality=e.target.value;view?.setQuality(currentQuality);};$('#reduce-motion').onchange=e=>{if(view)view.reduced=e.target.checked;};};
