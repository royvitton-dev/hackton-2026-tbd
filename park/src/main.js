...
paths.wrench='M14 6a5 5 0 0 0-6 6l-5 5a2 2 0 0 0 4 4l5-5a5 5 0 0 0 6-6l-3 3-3-3 3-3Z';
paths.parking='M5 3h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2ZM9 17V7h4a3 3 0 0 1 0 6H9';
const themeIcon=t=>({bumper:'car',pitstop:'wrench',parking:'parking',theater:'film',music:'mic'}[t]||'star');
...
<section id="cinema-ui" class="cinema-ui" hidden><div class="cinema-top"><button id="leave-cinema" class="cinema-back">${icon('arrow')} 파크로 돌아가기</button><div><span>STARLIGHT CINEMA</span><h2>지금, 우리의 이야기</h2></div><span class="cinema-badge">PRIVATE SCREENING</span></div><div class="cinema-bottom"><div><small>NOW PLAYING</small><h3>WONDER PARK · Eight Wonders, One Park</h3><p>8개 어트랙션의 실제 기능을 만나고, GS 폭죽 피날레로.</p></div><div class="film-controls"><button id="film-play" aria-label="영상 재생">${icon('play')}</button><span id="film-time">0:00 / 1:48</span><input id="film-seek" type="range" min="0" max="108" step=".1" value="0" aria-label="영상 재생 위치"><button id="film-mute" aria-label="영상 음소거">${icon('sound')}</button></div></div></section>
<video id="film" playsinline preload="metadata" crossorigin="anonymous" src="/api/project-asset/movie/output/wonder-park-tour.mp4" hidden></video>
...
function updateCatalog(next){
 ...
 $('#sync-label').textContent=['sites','vercel'].includes(state.hosting)?'온라인 파크 운영 중':state.watcher?(state.sync?.remote==='pending'?'새 커밋 확인 · 반영 대기':state.sync?.remote==='offline'?'로컬 운영 · 원격 재연결 대기':'10분마다 파크 업데이트'):'로컬 파크 운영 중';
...
function allAttractions(){setNav('attractions');showModal(`<div class="eyebrow">FIND YOUR NEXT ADVENTURE</div><h2>어디로 떠나볼까요?</h2><p>작은 건물마다 우리가 만든 이야기가 기다립니다. <a href="/projects/">모든 프로젝트 보기 ↗</a></p><div class="attraction-directory">${state.attractions.map(a=>`<button data-id="${escape(a.id)}"><span style="--accent:${a.color}">${icon(themeIcon(a.theme))}</span><div><small>${escape(a.english)}</small><strong>${escape(a.name)}</strong><p>${escape(a.description)}</p></div>${icon('arrow')}</button>`).join('')}</div>`);$('#modal').querySelectorAll('[data-id]').forEach(b=>b.onclick=()=>{closeModal();select(b.dataset.id);});}
function operations(){setNav('operations');const sync=state.sync;
 ...
 $('#refresh-now').onclick=async()=>{const b=$('#refresh-now');b.disabled=true;try{const r=await fetch('/api/refresh',{method:'POST'});if(!r.ok)throw new Error();if(['sites','vercel'].includes(state.hosting)){updateCatalog(await r.json());toast('현재 배포된 파크 정보를 확인했습니다.');}else toast(state.watcher?'커밋 확인을 요청했습니다. 잠시 뒤 운영실에 반영됩니다.':'감시 프로세스가 실행 중이지 않습니다. npm run park:watch로 시작하세요.');}catch{toast('파크 정보 확인에 실패했습니다.');}finally{b.disabled=false;}};
}
...
$('#castle-show').onclick=()=>{closeDetail();daylight(true);view?.focusCastle();toast(view?.reduced?'움직임 줄이기 설정으로 GS 불꽃을 정지 화면으로 보여드립니다.':'성 위에 GS 불꽃이 피어났다가 별빛처럼 사라집니다.');};
...
const events=['sites','vercel'].includes(state.hosting)?null:new EventSource('/api/events');if(events){events.onmessage=e=>{try{updateCatalog(JSON.parse(e.data));}catch{}};events.onerror=()=>{connectionError=true;$('#sync-label').textContent='파크 연결 복구 중';};}
