...
`npm run vehicle:start`로 실행 중인 `.next`를 다시 빌드하면 이전 HTML이 삭제된 JavaScript 파일을 참조해 404가 발생할 수 있습니다. 이 경우 서버를 재시작하고 브라우저를 새로고침합니다. 차량 확인 링크는 http://localhost:5190/vehicle/?user=U0001 입니다. 현재 20개 차량 프로필에 GLB를 연결했습니다. 16개는 외부에서 확보한 모델, BMW i5·Audi Q4/Q6·MINI 4개는 프로젝트에서 제작한 개략 모델입니다. 후자는 정밀 실차 형상이나 사진 수준의 품질에 도달한 모델이 아닙니다. 장면 안내와 출처에서 이를 구분합니다.
